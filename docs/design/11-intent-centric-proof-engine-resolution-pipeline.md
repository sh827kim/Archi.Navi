...
기존 function/callsite 기반 singular call proof를 담당한다.
...
- active proof confidence는 workspace default inference profile의 `proofConfidenceConfig`에서 계산한다.
...
- path-only unique match 예외는 이 파이프라인이 아니라 `HTTP_GATEWAY_ROUTE_INTENT`에 속한다.

## 4.2 `HTTP_GATEWAY_ROUTE_INTENT`

config-only gateway route family proof를 담당한다.
이 intent는 단일 endpoint 대신 “도달 가능한 endpoint 집합”을 먼저 좁히고,
그 뒤 endpoint-scoped child proof를 생성한다.
...
- route-family seed 자체는 candidate를 만들지 않는다.
- path-only unique match는 child proof derivation 규칙으로만 허용한다.
...
- 다수 child proof가 모두 frontier면 route-family root도 `FRONTIER`
...
- `READ`/`WRITE(service -> db_table)`는 `CLOSED_ATOMIC` 이후에만 projection한다.
...
- `BROKER_BINDING_UNRESOLVED`
- `MESSAGE_TARGET_UNRESOLVED`
- `TOPIC_MATCH_AMBIGUOUS`
- `MESSAGE_CONTRADICTION`

규칙:

- `produce`/`consume(service -> topic|queue)`는 `CLOSED_ATOMIC` 이후에만 projection한다.
...
agent가 생성한 `function_summary_patch`도 extractor와 동일한 summary 계약을 따라야 한다.
즉 `signalSources`, `extractionStrategy`, `summaryCompleteness`를 함께 남겨야 한다.
...
검증 실패 patch는 `REJECTED`로 기록되고 proof state는 frontier 유지.
...
confidence는 candidate를 만들기 위한 값이 아니라,
proof 상태가 닫혔을 때 그 닫힘의 신뢰도를 표현한다.
...
- confidence가 높아도 contradiction가 치명적이면 `REJECTED`
