...
describe('scanPythonAst — Python AST 스캐너 (Phase 2)', () => {
    ...
    it('@app.route에서 expose 신호를 추출해야 한다 (Phase 2: confidence 0.9)', async () => {
        const content = `
...
`;
    ...
    it('@app.get/@app.post/@app.put/@app.delete 모두 추출해야 한다', async () => {
        const content = `
@app.get('/api/orders')
def get_orders(): pass
...
`;
    ...
    it('@router.get 형태도 expose 신호를 추출해야 한다', async () => {
        const content = `
@router.get('/api/items')
def get_items(): pass
`;
        const result = await scanPythonAst('/src/routes.py', content);
...
`;
    ...
    it('상수 토픽을 추적하여 produce 신호를 추출해야 한다', async () => {
        const content = `
...
`;
    ...
    it('requests.get에서 call 신호를 추출해야 한다 (Phase 2: confidence 0.85)', async () => {
    ...
    it('requests.post/put/delete도 call 신호를 추출해야 한다', async () => {
        const content = `
...
`;
    ...
    it('빈 파일은 빈 signals 배열을 반환해야 한다', async () => {
        const result = await scanPythonAst('/src/empty.py', '');
        expect(result.signals).toHaveLength(0);
    });

    it('language는 항상 python이어야 한다', async () => {
        const result = await scanPythonAst('/src/app.py', `requests.get('http://api/v1')`);
